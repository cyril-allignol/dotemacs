...
** Mini-buffer
   ...
   #+BEGIN_SRC emacs-lisp +n
     (use-package helm-icons
       :config
       (helm-icons-enable)
       (setq helm-incons-provider 'all-the-icons)
       )
...
* Programming
** Compilation
   Automatically: kill existing compilation process, save buffers, jump to first
   error
#+BEGIN_SRC emacs-lisp +n
  (setq-default
...
#+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp +n
     (use-package company-coq
       ...
       (setq company-coq-completion-predicate nil)
       (setq company-idle-delay nil ;; on-demand completion
             company-minimum-prefix-length 0)
...
** Python
   ...
   most recent Linux systems) and a few symbols:
   #+BEGIN_SRC emacs-lisp +n
     (use-package python
       :mode ("\\.py\\'" . python-mode)
       :config
